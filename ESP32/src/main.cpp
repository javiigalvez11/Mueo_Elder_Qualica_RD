...
void setup()
{
  ...
  W5500::begin(); // W5500 (usa config cargada)

  rele_begin(); // relé

  GM65::begin(); // lector QR GM65
...
static bool validateAndOpen(const String &code, uint32_t validateTimeoutMs)
{
  ...
  if (!linkUp())
  {
    ...
    if (vr == 1)
    {
      ...
      if (debugSerie)
      {
        Serial.println("[IO][OFFLINE] Ticket=" + code + " valido, guardado en memoria");
      }
      logbuf_pushf("[IO][OFFLINE] Ticket=%s valido, guardado en memoria", code.c_str());
      activaConecta = 1;
      return true;
    }
    ...
    if (debugSerie)
    {
      ...
      logbuf_pushf("[IO][OFFLINE] Ticket=%s aún no es hora → rechazo", code.c_str());
      if (vr == 3)
        Serial.println("[IO][OFFLINE] Ticket" + code + " no existe → rechazo");
      logbuf_pushf("[IO][OFFLINE] Ticket=%s no existe → rechazo", code.c_str());
      return false;
    }
    pasoActual = -1;
    entradaPendiente.remove(0);
  ...
  {
    if (debugSerie)
      Serial.println("[IO] ERROR: no se pudo encolar VALIDATE");

    logbuf_pushf("[IO] ERROR: no se pudo encolar VALIDATE para Ticket=%s", code.c_str());
  }
  ...
  const uint32_t maxWait = min<uint32_t>(validateTimeoutMs, (uint32_t)10000);
  ...
  if (autorizado)
  {
    ...
    logbuf_pushf("[IO] ONLINE Ticket=%s autorizado → abrir", code.c_str());
  ...
  // Timeout de VALIDATE (no es timeout de PASO)
  if (debugSerie)
    Serial.println("[IO] ONLINE validate timeout → denegar (sin validatePass)");

  pasoActual = 0; // denegado por timeout de validación
  activaEntrada = 0;
  entradaPendiente.remove(0);
  activaConecta = 1;
  return false;
...
static void taskIO(void *pv)
{
  for (;;)
  {
    ...
    else
    {
      ...
      String code;
      if (GM65::readLine_parsed(code) && activaConecta)
      {
        if (debugSerie)
        {
...
static void realizarPaso(const String &code)
{
  ...
  // 2. Cargamos la config actual, actualizamos el valor y guardamos
  TornoConfig c = cfgLoad();
  c.aperturasTotales = aperturasTotales;
  cfgSave(c);
